...
#
# Builds and pushes the certspotter image (multi-arch) tagged with
# the version semantic-release just computed, then generates one SPDX SBOM
# per platform — a multi-arch manifest list has different packages per
# platform, so the list digest itself isn't a meaningful SBOM scan target.
#
# Published as ghcr.io/merkleye/certspotter — a package scoped to this repo,
# not the ghcr.io/merkleye/merkleye-certspotter package the old monorepo
# published (and whose GHCR access list this repo's default token has no
# reason to be on).
#
...
OCI_SOURCE="https://github.com/merkleye/certspotter"
